...
def doppler_shift(distance, temperature, velocity, LOSvelocity, time, min_frequency, max_frequency):
    ...
    v1par = LOSvelocity
    t = time
    nu_min = min_frequency
    nu_max = max_frequency
    ...
    # Frequency
    nu = np.linspace(nu_min, nu_max, num=500)
    lamda = []
    ...
    for j in range (len(t)):
        for k in range (len(lamda)):
            ...
            z2_prime.append(c**2)
            z3_prime.append(h * nu_prime[j,k] / (kb * T[k]))
    ...
    N_prime = np.zeros((len(t), len(lamda) - 1))   #Doppler shifted number of photons
    Ndot_prime = np.zeros((len(t), len(lamda) - 1))   #Doppler shifted photon flux
...
rs_arr = np.linspace(r_sch, rs, num=500)

# Temperatures
T = []   #temperature of minidisk [K]
T0 = 6e5 * (m1 / (1e8 * m_sun))**(0.25)   #inner minidisk temperature [K]
for j in range (len(rs_arr)):
    T.append(T0 * (rs_arr[j] / r_sch)**-0.75)
